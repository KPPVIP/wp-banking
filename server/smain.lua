...
RegisterServerEvent("WPCore:Server:OnPlayerLoadedFromDB")
AddEventHandler('WPCore:Server:OnPlayerLoadedFromDB', function(src)
...
WPCore.Functions.CreateCallback('bb-banking:server:getPlayerData', function(source, cb, group)
	...
	local Player = WPCore.Functions.GetPlayer(source)
	...
	WPCore.Functions.ExecuteSql(true, "SELECT * FROM `bbanking_accounts` WHERE `identifier` = '"..playerData.information.iban.."'", function(result)
		...
		WPCore.Functions.ExecuteSql(true, "SELECT * FROM `bbanking_cards` WHERE `identifier` = '"..playerData.information.iban.."'", function(result2)
			...
			WPCore.Functions.ExecuteSql(true, "SELECT * FROM `bbanking_statements` WHERE `iban` = '"..playerData.information.iban.."' ORDER BY `time` DESC LIMIT 1024", function(result3)
...
AddEventHandler('bb-banking:server:withdrawEvent', function(data)
	...
	local Player = WPCore.Functions.GetPlayer(src)
	...
	if Player then
		if account == "account" or account == "atm" then
			if tonumber(Player.PlayerData.money["bank"]) >= amount then
				...
				TriggerClientEvent('WPCore:Notify', src, "Insufficient Bank Funds", "error")
			end
		elseif account == "saving" then
			WPCore.Functions.ExecuteSql(true, "SELECT * FROM `bbanking_accounts` WHERE `identifier` = '"..Player.PlayerData.citizenid.."'", function(result)
				if(result[1] ~= nil and result[1].amount >= amount) then
					...
					WPCore.Functions.ExecuteSql(true, "UPDATE `bbanking_accounts` set amount = amount - " .. amount .. " WHERE `identifier` = '"..Player.PlayerData.citizenid.."'", function()
						Citizen.Wait(0)
						TriggerClientEvent("bb-banking:client:refreshNuiNew", src, false)
					end)
				else
					TriggerClientEvent('WPCore:Notify', src, "Insufficient Saving Bank Funds", "error")
				end		
			end)
		end
	end
	...
	if Player then
		if account == "account" then
			if tonumber(Player.PlayerData.money["cash"]) >= amount then
				...
				TriggerClientEvent('WPCore:Notify', src, "Insufficient Cash", "error")
			end
		elseif account == "saving" then
			if tonumber(Player.PlayerData.money["bank"]) >= amount then
				WPCore.Functions.ExecuteSql(true, "UPDATE `bbanking_accounts` set amount = amount + " .. amount .. " WHERE `identifier` = '"..Player.PlayerData.citizenid.."'", function()
...
AddEventHandler('bb-banking:server:transferEvent', function(data)
	...
	local Player = WPCore.Functions.GetPlayer(source)
	local targetPlayer = WPCore.Functions.GetPlayer(target)
	...
	if Player and targetPlayer then
		if tonumber(Player.PlayerData.money["bank"]) >= amount then
			...
			TriggerClientEvent('WPCore:Notify', source, "Insufficient Bank Funds", "error")
		end
	else
		TriggerClientEvent('WPCore:Notify', source, "User ID not found!", "error")
	end
end)
	...
	elseif event == "sell" then
		...
		if Player then
			if tonumber(Player.PlayerData.money["crypto"] * cryptoWorth) >= cashAmount then
				...
				TriggerClientEvent('WPCore:Notify', source, "Insufficient Crypto Worth", "error")
			end
		else
			TriggerClientEvent('WPCore:Notify', source, "User ID not found!", "error")
		end
...
AddEventHandler('bb-banking:server:createSavingAccount', function()
	...
	if Player then
		WPCore.Functions.ExecuteSql(true, "INSERT INTO `bbanking_accounts` (`identifier`, `type`, `amount`) VALUES ('"..Player.PlayerData.citizenid.."', 'saving', 0)")
	end
end)
...
AddEventHandler('bb-banking:server:checkCards', function()
	...
	if Player then
        ...
        WPCore.Functions.ExecuteSql(true, "SELECT * FROM `bbanking_cards` WHERE `identifier` = '"..Player.PlayerData.citizenid.."'", function(result)
			if(result[1] ~= nil) then
...
AddEventHandler('bb-banking:server:createCard', function(data)
	...
	if Player then
		...
		WPCore.Functions.ExecuteSql(true, "INSERT INTO `bbanking_cards` (`identifier`, `holder`, `type`, `number`, `pin`, `hold`, `data`) VALUES ('"..Player.PlayerData.citizenid.."', '".. ccHolder .."', 'account', '".. ccNumber .."', '".. ccPin .."', 0, '".. json.encode(data) .."')", function(result)
			TriggerClientEvent("bb-banking:client:refreshNuiNew", src, false)
		end)
	end
end)
...
AddEventHandler('bb-banking:server:cardEvent', function(data)
	...
	if Player then
		if data.action == "remove" then
			...
			WPCore.Functions.ExecuteSql(true, "DELETE FROM `bbanking_cards` WHERE `id` = " .. data.card.id .. " AND `identifier` = '" .. Player.PlayerData.citizenid .. "'", function(result)
				TriggerClientEvent("bb-banking:client:refreshNuiNew", src, false)
			end)
		elseif data.action == "disable" then
			-- Always use his identifier to prevent hackers from deleting other people's cards.
			WPCore.Functions.ExecuteSql(true, "UPDATE `bbanking_cards` SET hold = 1 WHERE `id` = " .. data.card.id .. " AND `identifier` = '" .. Player.PlayerData.citizenid .. "'", function(result)
				TriggerClientEvent("bb-banking:client:refreshNuiNew", src, false)
			end)
		elseif data.action == "activate" then
			-- Always use his identifier to prevent hackers from deleting other people's cards.
			WPCore.Functions.ExecuteSql(true, "UPDATE `bbanking_cards` SET hold = 0 WHERE `id` = " .. data.card.id .. " AND `identifier` = '" .. Player.PlayerData.citizenid .. "'", function(result)
				TriggerClientEvent("bb-banking:client:refreshNuiNew", src, false)
			end)
		end
	end
...
AddEventHandler('bb-banking:server:RegisterNewAction', function(src, moneytype, actiontype, amount, reason)
	if moneytype == "bank" then
		...
		if Player then
			WPCore.Functions.ExecuteSql(true, "INSERT INTO `bbanking_statements` (`iban`, `from`, `source`, `type`, `amount`, `reason`, `time`) VALUES ('" .. Player.PlayerData.citizenid .. "', 'ACTION_UNKNOWN', 'account', '" .. actiontype .. "', '" .. amount .. "', '" .. reason .. "', '" .. os.time() .. "')", function(result)
				TriggerClientEvent("bb-banking:client:refreshNuiNew", src, false)
			end)
		end
	end
